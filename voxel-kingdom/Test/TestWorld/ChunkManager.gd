...
@export_range(0.0, 1.0) var mountain_biome_threshold: float = 0.8
...
@export var lava_pool_threshold: float = 0.8
...
func _get_needed_grid_y_values(center_chunk_grid_y: int, grid_x: int, grid_z: int, is_player_column: bool = false) -> Array[int]:
	...
	if is_player_column and center_chunk_grid_y > surface_grid_y:
		for grid_y: int in range(surface_grid_y, center_chunk_grid_y + 1):
			if not grid_y_values.has(grid_y):
				grid_y_values.append(grid_y)
...
func _queue_needed_chunks(center_chunk_grid: Vector3i) -> void :
	...
	for grid_x: int in range(center_chunk_grid.x - view_distance_in_chunks, center_chunk_grid.x + view_distance_in_chunks + 1):
		for grid_z: int in range(center_chunk_grid.z - view_distance_in_chunks, center_chunk_grid.z + view_distance_in_chunks + 1):
			...
			var is_player_column: bool = grid_x == center_chunk_grid.x and grid_z == center_chunk_grid.z
			var grid_y_values: Array[int] = _get_needed_grid_y_values(center_chunk_grid.y, grid_x, grid_z, is_player_column)
			...
			for grid_y: int in grid_y_values:
				...
				_chunk_vertical_priority[chunk_world_key] = absi(grid_y - surface_grid_y)
	...
	var vertical_priority_map: Dictionary[Vector3i, int] = _chunk_vertical_priority

	load_queue.sort_custom( func(first_key: Vector3i, second_key: Vector3i) -> bool:
		var first_grid: Vector3i = Vector3i(int(first_key.x / float(chunk_size)), int(first_key.y / float(chunk_size)), int(first_key.z / float(chunk_size)))
		var second_grid: Vector3i = Vector3i(int(second_key.x / float(chunk_size)), int(second_key.y / float(chunk_size)), int(second_key.z / float(chunk_size)))

		var first_in_column: bool = first_grid.x == center_chunk_grid.x and first_grid.z == center_chunk_grid.z
		var second_in_column: bool = second_grid.x == center_chunk_grid.x and second_grid.z == center_chunk_grid.z

		if first_in_column != second_in_column:
			return first_in_column

		if first_in_column and second_in_column:
			var first_below: bool = first_grid.y <= center_chunk_grid.y
			var second_below: bool = second_grid.y <= center_chunk_grid.y
			if first_below != second_below:
				return first_below
			return absi(first_grid.y - center_chunk_grid.y) < absi(second_grid.y - center_chunk_grid.y)


		var first_offset: Vector2i = Vector2i(first_grid.x - center_chunk_grid.x, first_grid.z - center_chunk_grid.z)
		var second_offset: Vector2i = Vector2i(second_grid.x - center_chunk_grid.x, second_grid.z - center_chunk_grid.z)

		var first_distance: int = maxi(absi(first_offset.x), absi(first_offset.y))
		var second_distance: int = maxi(absi(second_offset.x), absi(second_offset.y))
		...
		var first_vertical_priority: int = vertical_priority_map.get(first_key, 0)
		var second_vertical_priority: int = vertical_priority_map.get(second_key, 0)

		if first_vertical_priority != second_vertical_priority:
			return first_vertical_priority < second_vertical_priority


		var first_alignment: float = Vector2(first_offset.x, first_offset.y).normalized().dot(movement_direction) if first_offset != Vector2i.ZERO else 1.0
		var second_alignment: float = Vector2(second_offset.x, second_offset.y).normalized().dot(movement_direction) if second_offset != Vector2i.ZERO else 1.0
...
func _dispatch_load_queue() -> void :
	...
	for index: int in range(count):
		...
		_chunk_vertical_priority.erase(chunk_world_key)
...
func add_voxel_at_position(target_block: Vector3i, voxel_type: TerrianData.TerrianType) -> void :
	...
	if TerrianData.is_fallable(voxel_type):
		_drop_if_needed(target_block)
...
func remove_voxel_at_position(target_block: Vector3i) -> void :
	...
	var local_voxel_position: Vector3i = target_block - chunk_world_key
	var voxel_type: TerrianData.TerrianType = chunk.chunk_data.get_voxel(local_voxel_position)

	chunk.remove_voxel_at_local(local_voxel_position)

	if voxel_type != TerrianData.TerrianType.BEDROCK and voxel_type != TerrianData.TerrianType.AIR:
		var world_pos: Vector3 = Vector3(
			chunk_world_key.x + local_voxel_position.x, 
			chunk_world_key.y + local_voxel_position.y, 
	...
	_cascade_gravity(target_block)


func _move_voxel(from_pos: Vector3i, to_pos: Vector3i) -> void :
	var voxel_type: TerrianData.TerrianType = get_voxel_type_at(from_pos)

	var to_chunk_key: Vector3i = _voxel_to_chunk_key(to_pos)
	chunks_mutex.lock()
	var to_chunk: Chunk = chunks_by_key.get(to_chunk_key)
	chunks_mutex.unlock()
	if to_chunk != null:
		to_chunk.set_voxel(to_pos - to_chunk_key, voxel_type)
		_store_modified_chunk(to_chunk_key, to_chunk)

	var from_chunk_key: Vector3i = _voxel_to_chunk_key(from_pos)
	chunks_mutex.lock()
	var from_chunk: Chunk = chunks_by_key.get(from_chunk_key)
	chunks_mutex.unlock()
	if from_chunk != null:
		from_chunk.remove_voxel_at_local(from_pos - from_chunk_key)
		_store_modified_chunk(from_chunk_key, from_chunk)


func _cascade_gravity(pos: Vector3i) -> void :
	var above: Vector3i = pos + Vector3i.UP
	var above_type: TerrianData.TerrianType = get_voxel_type_at(above)

	if not TerrianData.is_fallable(above_type):
		return

	_move_voxel(above, pos)
	_drop_if_needed(pos)
	_cascade_gravity(above)


func _drop_if_needed(pos: Vector3i) -> void :
	var current_type: TerrianData.TerrianType = get_voxel_type_at(pos)
	if not TerrianData.is_fallable(current_type):
		return
...
func _spawn_block_break_particles(world_pos: Vector3, type: TerrianData.TerrianType) -> void :
	call_deferred("_spawn_block_break_particles_deferred", world_pos, type)


func _spawn_block_break_particles_deferred(world_pos: Vector3, type: TerrianData.TerrianType) -> void :
	var particles: = block_break_particles_scene.instantiate() as BreakParticles
	effects.add_child(particles)
	particles.emit_at(world_pos, type)

	var lifetime: float = particles.lifetime
